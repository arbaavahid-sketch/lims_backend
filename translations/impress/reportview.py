...
<div class="report ${report_class}" dir="${report_dir}" lang="${report_language}" uids="${uids}" client_uid="${client_uid}">
...
class ReportView(Base):
    ...
    EN_REPORT_TERMS = {
        ...
        u"کشش‌پذیری قیر (داکتیلیتی ، D113)": u"Bitumen ductility (D113)",
        u"اندازه گیری آب و رسوبات به روش سانتریفیوژ": u"Water and sediment by centrifuge",
        u"گرانروی کینماتیک در دمای 100 درجه سانتی گراد و با سرعت برش کم": u"Kinematic viscosity at 100°C, low shear rate",
        u"گرانروی کینماتیک در دمای 150 درجه سانتی گراد و با سرعت برش زیاد": u"Kinematic viscosity at 150°C, high shear rate",
        u"گرانروی کینماتیک مایعات در دمای 100 درجه سانتی گراد": u"Kinematic viscosity of liquids at 100°C",
        u"گرانروی در 40 درجه سانتی گراد": u"Viscosity at 40°C",
        u"گرانروی کینماتیک مایعات در دمای 40 درجه سانتی گراد": u"Kinematic viscosity of liquids at 40°C",
    ...
    def get_report_language_context(self):
        ...
        return {
            ...
            "report_dir": is_rtl and "rtl" or "ltr",
            "report_class": is_rtl and "report-fa" or "report-en",
        }

    def is_report_english(self):
        language = (self.request.get("LANGUAGE", "") or "en").split("-")[0]
        return language != "fa"

    def to_report_unicode(self, value):
    ...
    def normalize_report_text(self, value):
        ...
        return re.sub(ur"\s+", u" ", text, flags=re.UNICODE).strip()

    def get_en_report_terms(self):
        if self.EN_REPORT_TERMS_NORMALIZED is None:
            self.EN_REPORT_TERMS_NORMALIZED = dict(
                (self.normalize_report_text(key), value)
                for key, value in self.EN_REPORT_TERMS.items())
        return self.EN_REPORT_TERMS_NORMALIZED

    def translate_report_text(self, value):
        if not value or not self.is_report_english():
            return value
        text = self.to_report_unicode(value)
        normalized = self.normalize_report_text(text)
        translation = self.get_en_report_terms().get(normalized)
        if translation:
            return translation
...
class SingleReportView(ReportView):
    ...
    def get_template_context(self, model, **kw):
        ...
        context.update(self.get_report_language_context())
        context.update(kw)
        return context
